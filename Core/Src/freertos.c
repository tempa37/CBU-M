...
volatile TaskHandle_t uart_test_task = NULL;
volatile UART_HandleTypeDef *uart_test_tx_uart = NULL;
volatile UART_HandleTypeDef *uart_test_rx_uart = NULL;
volatile uint16_t uart_test_rx_size = 0;
...
static void uart_test_run_once(UART_HandleTypeDef *tx_uart, UART_HandleTypeDef *rx_uart, uint8_t tx1, const uint8_t *packet, uint16_t len, const char *name, uint8_t *ok) {
  ...
  uart_test_task = xTaskGetCurrentTaskHandle();
  ...
  if (HAL_UART_Receive_IT(rx_uart, rx_buf, len) != HAL_OK) {
    *ok = 0;
    snprintf(uart_test_message, sizeof(uart_test_message), "%s: ошибка старта RX", name);
    goto done;
  }

  if (HAL_UART_Transmit_IT(tx_uart, (uint8_t *)packet, len) != HAL_OK) {
    *ok = 0;
    snprintf(uart_test_message, sizeof(uart_test_message), "%s: ошибка старта TX", name);
    goto done;
  }

  if (xTaskNotifyWait(0, UINT32_MAX, &notified, pdMS_TO_TICKS(1000)) != pdTRUE || (notified & UART_TEST_EVT_TX_DONE) == 0) {
    *ok = 0;
    snprintf(uart_test_message, sizeof(uart_test_message), "%s: таймаут TX", name);
    goto done;
  }

  if (xTaskNotifyWait(0, UINT32_MAX, &notified, pdMS_TO_TICKS(1000)) != pdTRUE || (notified & UART_TEST_EVT_RX_DONE) == 0) {
    *ok = 0;
    snprintf(uart_test_message, sizeof(uart_test_message), "%s: таймаут RX", name);
    goto done;
  }

  if (uart_test_rx_size != len) {
    *ok = 0;
  ...
  uart_test_task = NULL;
  uart_test_tx_uart = NULL;
  uart_test_rx_uart = NULL;
  uart_test_rx_size = 0;
}
